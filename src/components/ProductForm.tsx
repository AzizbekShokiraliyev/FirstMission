...
export const ProductForm = ({ initialData, onSubmit, isLoading, title }: ProductFormProps) => {
      ...
      >
        <div className="grid flex-1 auto-rows-min gap-5 overflow-y-auto px-4">
          <div className="grid gap-2">
            ...
            <Input id="name" placeholder="Mahsulot nomini yozing" className="text-white" {...register("name")} />
            {errors.name && <p className="text-xs font-medium text-red-500">{errors.name.message}</p>}
          </div>

          <div className="grid gap-2">
            <Label htmlFor="description" className="text-white">Description</Label>
            <Input id="description" placeholder="Tavsif yozing mahsulot uchun" className="text-white" {...register("description")} />
          ...
          <div className="grid gap-2">
            ...
            {errors.count && <p className="text-xs font-medium text-red-500">{errors.count.message}</p>}
          </div>
        ...
        <SheetFooter className="border-t border-slate-800 pt-4">
          <Button type="submit" className="bg-blue-700 text-white hover:bg-blue-800" disabled={isLoading}>
